...
## Daylight amendment 3 (11 Jul 2026): live "Session already started" during separation

**Symptom:** on a clean slate, a separation would start, progress partway, then fail with "SEPARATION FAILED — Session already started".

**Root cause:** re-entrancy with no guard at any level. The worker's onmessage handler is async, so a second `separate` request interleaves with a running loop instead of waiting, and both call `session.run()` on the one ORT session, which is exactly the condition ORT reports as "Session already started". Nothing upstream prevented the second request: `Separator.separate()` had no in-flight check, and on the desk there is a window between pressing SEPARATE and the phase change hiding the control (the async model-cache probe) in which a second press lands. The per-chunk watchdog then amplified the damage, each colliding loop tearing down and recreating the session under the other.

**Fix, three layers:**
1. **Worker (root cause):** a `SerialQueue` (unit-tested: max concurrency one, order preserved, a failing task never poisons the queue) now serialises everything that touches the session, both warmup and separate. A separate request arriving mid-run also raises the cancel flag, so the running loop stops at its next chunk boundary and the queued run starts promptly and clean; the cancel flag is reset only when the previous run has actually ended, and `cancel` itself stays un-queued so it can reach the run in flight. Session lifecycle is therefore strictly idle to running to settled, with no overlapping transitions.
2. **Orchestrator:** `Separator.separate()` is idempotent while running; a re-entrant call joins the in-flight promise rather than starting a competing run, and a finally always clears the in-flight state on done, cancel, or error, so retry never needs a reload.
3. **Desk:** `startSeparation` carries a busy guard spanning the whole click-to-phase-change window including the model probe, cleared in a finally on every outcome, and the SEPARATE control keeps hiding on phase change as before.

Recovery was checked end to end: any failure lands in phase `error`, which re-offers the SEPARATE control, the worker keeps its (still healthy) session or recreates it lazily on the next run, and the partials store means a retry resumes rather than restarts. The Night 1 ORT recipe is untouched.

**Tests:** four SerialQueue unit tests (116 unit total) plus an e2e that double-presses SEPARATE and asserts one status readout, clean completion, no error, and a single cache entry. One real integration run confirmed the queued path against actual ORT. One test-design finding recorded: double-clicking GO after a cancel is not a re-entrancy case, because the second click lands on the STOP control that replaces GO at the same desk position and legitimately pauses again; recoverable, positional, left as is.

---

## Daylight amendment 4 (11 Jul 2026): COEP rejected the cached model response

**Symptom:** on the deployed site, the model fetch from R2 failed under COEP with the response showing status 200 "from service worker" and `Cross-Origin-Resource-Policy: same-origin`.

**Root cause: our own synthesised header, not R2 and not the service worker's fetch mode.** The critical clue was that the ORT files load fine from the same bucket: the service worker caches those by cloning the real network response, a CORS-type response (cross-origin fetch defaults to cors mode and the bucket's CORS policy allows the origin), and a replayed CORS response satisfies COEP with no CORP header at all. The model takes a different path: the page consumes the body for the SHA-256 check, so the cached entry is a synthesised `Response` whose headers we write ourselves, and it carried `CORP: same-origin`, a leftover from when the model URL was same-origin dev middleware. When the service worker later served that entry for the separation worker's fetch of the cross-origin R2 URL, the browser rejected it exactly as COEP requires: same-origin CORP on a cross-origin resource. R2 itself sends no CORP header; the one in the Network tab was ours, so no bucket or dashboard configuration is needed beyond the existing CORS policy.

**Fix:** the synthesised cache entry now carries `CORP: cross-origin` (correct for a resource consumed cross-origin; irrelevant in dev where the URL is same-origin), the download fetch is explicitly `mode: "cors"`, and `ensure()` repairs entries written by the broken build in place, rewriting headers without re-downloading 166MB, so already-affected visitors recover on their next press with no cleared site data. Three new unit tests pin the cached CORP header, the CORS fetch mode, the in-place repair, and that a corrupt download caches nothing (119 unit total). Nothing else changed: ORT from R2, the offline PWA promise, and the Night 1 recipe are untouched.

---

## Daylight amendment 5 (11 Jul 2026): the Night 2 watchdog raced its own session

**Symptom:** with the model loading fixed, "SEPARATION FAILED — Session already started" returned, now mid-run at a consistent ~46% on a clean single press.

**Root cause: the per-chunk watchdog's recovery, not a second request.** The Night 2 watchdog guarded each chunk with a `Promise.race` timeout; when it fired, the losing `session.run` was still pending (a hung run cannot be cancelled or awaited), yet the recovery released the session, created a new one, and ran the retry alongside the still-pending run, which is exactly the condition ORT reports as "Session already started". The Night 5 SerialQueue could not help: the collision happens inside one queued task. The consistent ~46% failure point means a chunk genuinely stalled there (matching the Night 2 observation of GPU device loss on the Intel iGPU after a long run of dispatch-heavy chunks), so the watchdog fired for a real reason and its unsafe recovery turned a recoverable stall into a fatal error. The Night 2 benchmarks never tripped it because they ran the pre-optimised model on a thermally supervised machine.

**Fix: recovery now splits by whether the failing run has settled.**
- **In-worker:** the race timeout is gone. The chunk retry (release, recreate, run once more) now happens only in the catch of a run that has actually rejected, which is the only moment the session is provably idle. Every create/release/recreate path is audited in a comment on `runChunkWithRecovery`.
- **Orchestrator:** hangs are handled where they can be handled safely. An inactivity watchdog in `runWorker` treats every worker message as proof of liveness; silence past the deadline terminates the entire worker (process-level teardown, nothing left to race), awaits the in-flight partial writes, respawns a fresh worker, and resumes once from the persisted partials, so the stalled song completes from the last finished chunk rather than dead-ending. A second hang on the same song surfaces as an honest error, and the SEPARATE control re-offers a retry that again resumes from partials.
- **Timeout posture:** the deadline floor is 240s, comfortably above a cold session create (~120s worst) and the slowest observed CPU chunk (~35s), stretching to eight times the measured average chunk time as real numbers arrive; `?sepWatchdogMs=` overrides it for tests. A spurious fire is now safe rather than fatal: it costs one worker respawn and session re-create, then resumes.

**Tests:** a new stall mode in the mock worker (`?mockSeparation=stall`) goes silent after three chunks on a fresh run, and an e2e drives the full recovery: progress, silent stall, watchdog termination, respawn, resume from partials, clean completion with no error and a normal cache entry. One real integration run confirmed the reworked chunk loop and watchdog-monitored path against actual ORT (46.6s, green). 119 unit and 30 chromium e2e all green; the Night 1 recipe is untouched.

---

## Daylight amendment 6 (12 Jul 2026): the watchdog was armed during warmup

**Symptom:** regression after the amendment 5 watchdog: separation sat at 0% with no ETA and never progressed, where it previously reached ~46% before failing.

**Root cause: the inactivity watchdog was armed from the moment SEPARATE was pressed, but warmup is legitimately silent for longer than any chunk deadline can tolerate.** Cold-start arithmetic, made explicit: after the page-side model download (its own progress LCD, never watchdog scope), the worker performs its own model fetch (seconds from the service-worker cache, but minutes if that fetch ever goes direct to the network), then a WebGPU session-create attempt bounded at 120s, and on failure a WASM attempt bounded at another 120s. Those creates are near-silent: a handful of warming messages at attempt boundaries and nothing between. Two chained attempts plus the fetch can legitimately exceed the 240s deadline, and when the watchdog fired mid-create it terminated the worker and respawned it into another equally slow create: a kill loop that presents as a desk frozen at 0% with no ETA, which is exactly the report. Warmup hangs also never needed the watchdog: `CREATE_TIMEOUT_MS` in the worker already settles a hung create into an honest error.

**Fix:** the watchdog now arms only on the `warm` message, when chunk processing begins, and can never fire during warmup, session creation, or the in-worker model load. The first chunk gets a doubled grace (480s floor: WebGPU pipeline compilation and the WASM thread-pool spin-up land in chunk one), and the deadline then tracks eight times the measured average chunk time. The worker additionally posts a liveness heartbeat every 10s while a create is in flight, so warming silence is diagnosable rather than ambiguous.

**Verified against a real separation, not just mocks**, with per-phase timings on this hardware (headless, so the CPU path): fixture decode 0.3s; model fetch plus hash 1.5s; warmup wholly silent for 8.5s with the watchdog unarmed; chunk phase 28.5s against a 480s first-chunk grace; four stems, reconstruction error 3.3%, zero desk errors. Production cold starts stretch these numbers (40 to 120s creates on modest hardware) but stay orders of magnitude inside the grace, and warmup length is now irrelevant to the watchdog by construction. Two e2e flows pin the behaviour: the existing stalled-chunk recovery (watchdog still fires and resumes during the chunk phase) and a new slow-warm mode (`?mockSeparation=slowwarm`) that warms silently for four times the test watchdog deadline and must complete cleanly. 119 unit, 30 chromium e2e, and the offline build test all green.
